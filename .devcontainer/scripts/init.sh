...
# ===== Variables (via devcontainer.json) =====
...
# ===== Pré-requis système (suivant docs/development) =====
log "Install des paquets requis (build tools, MariaDB, Redis, wkhtmltopdf, fonts, Node/Yarn)"
...
# Yarn 1.x (Frappe v15 utilise encore yarn classic)
if ! command -v yarn >/dev/null 2>&1; then
  sudo npm i -g yarn@1
fi

# pipx pour isoler frappe-bench (reco officielle)
...
# ===== Démarrer DB/Redis & initialiser le root MariaDB =====
...
# essaie sans mot de passe (fresh), puis avec (si déjà config)
...
# bind 127.0.0.1 (optionnel, plus sûr pour devcontainer)
...
log "Installation/upgrade frappe-bench (pipx)"
...
if [ ! -d "$HOME/frappe-bench" ]; then
  log "bench init --frappe-branch ${FRAPPE_BRANCH}"
  bench init --frappe-branch "${FRAPPE_BRANCH}" --python "$(command -v python3.11)" "$HOME/frappe-bench"
fi

cd "$HOME/frappe-bench"

# Injecter les creds DB root dans common_site_config (évite les prompts)
log "Maj sites/common_site_config.json"
python3 - <<'PY'
import json, os
...
# ===== Démarrer bench (background) =====
log "Démarrage de bench (background)"
nohup bench start >/tmp/bench.log 2>&1 &

log "OK → http://localhost:8000 (port 8000), site: ${SITE_NAME}, admin: ${ADMIN_PASSWORD}"
